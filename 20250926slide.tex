...
\begin{center}
\Huge{明日から\structure{graphlib}、みんなで使おう}
\end{center}
...
\begin{block}{グラフの定義}
集合$X$の$k$個の要素からなる部分集合全体の族を$\binom{X}{k}$と表す。
$E \subset \binom{V}{2}$を満たす集合の組$(V, E)$を\structure{グラフ}と呼ぶ。
このとき、$V=V(G)$を\structure{頂点集合}、$E=E(G)$を\structure{辺集合}と呼ぶ。
また、$V(G)$の要素をグラフ$G$の\structure{頂点}、$E(G)$の要素を$G$の\structure{辺}と呼ぶ。
\end{block}
