...
\begin{definition}[歩道]
グラフ$G=(V, E)$の頂点$u_{1}$から$u_{k}$への長さ$k$の\structure{歩道}$W$とは、頂点の列$(u_{1}, u_{2}, \dots, u_{k})$で、$(u_{i}, u_{j}) \in E (1 \leq i < j \leq k)$を満たすものである。特に、$u_{1}=u_{k}$の場合、$W$は\structure{閉じている}と呼ぶ。
\end{definition}
